...
interface WeatherDataByLocation {
  [key: string]: WeatherData;
}

interface FishingCondition {
  status: string;
  color: string;
}

const FishingWeatherApp: React.FC = () => {
  const [weather, setWeather] = useState<WeatherData | null>(null);
  const [location, setLocation] = useState<string>('San Francisco Bay');
  ...
  const mockWeatherData: WeatherDataByLocation = {
    'San Francisco Bay': {
      current: {
        temp: 68,
  ...
  const getFishingConditions = (waves: number, wind: number, visibility: number): FishingCondition => {
